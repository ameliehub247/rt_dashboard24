...
# --- Top Posts nach Reaktionstyp --- 
st.subheader("Top Posts nach Reaktionstyp (optional mit Suchbegriff)")

selected_reaction = st.selectbox(
    "Wähle einen Reaktionstyp aus:",
    reaction_columns
)

if not filtered_df_with_term.empty:
    st.markdown(f"#### Top Posts mit '{search_term}' nach '{selected_reaction}' Reaktionen")
    # Sortiere die gefilterten Posts nach der ausgewählten Reaktion absteigend
    ...
    if not top_posts.empty:
        ...
        st.info(f"Keine Posts mit '{search_term}' und '{selected_reaction}' gefunden.")
elif not df.empty:
    st.markdown(f"#### Top Posts gesamt nach '{selected_reaction}' Reaktionen (kein Suchbegriff aktiv)")
    top_posts = df.sort_values(by=selected_reaction, ascending=False).head(10)
    if not top_posts.empty:
        st.dataframe(top_posts[['text', selected_reaction]])
    else:
        st.info(f"Keine Posts mit '{selected_reaction}' gefunden.")
else:
    st.info("Keine Daten zum Anzeigen von Top Posts verfügbar.")
